...
async function handleStart(e) {
	...
	// Stop any playing voice preview before starting interview
...
// Multi-step form wizard - using window scope for cross-script access
window.currentFormStep = window.currentFormStep || 1;
const totalSteps = 3;


function nextStep() {
	if (!validateStep(window.currentFormStep)) {
		return;
	}

	if (window.currentFormStep < totalSteps) {
		window.currentFormStep++;
		showStep(window.currentFormStep);
	}
}


function previousStep() {
	if (window.currentFormStep > 1) {
		window.currentFormStep--;
		showStep(window.currentFormStep);
	}
}
